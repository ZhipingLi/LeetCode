/**
 * 该问题可以转化为「背包问题」的描述形式：
 * 有一个背包，最大容量为amount，有一系列物品coins，每个物品的重量为coins[i]，每个物品的数量无限（完全背包）。求有多少种方法，能够把背包恰好装满。
 * 
 * 「完全背包问题」就是每个物品能够被无限次放入的「背包问题」。
 * 二者之间的区别体现在背包能够放入物品（if(W - weight >= 0){ ... }）且放入物品时（dp(..., W - weight)）：
 *    「完全背包问题」：dp(i, W - weight) // 放入第i种物品后，还可以继续使用该种物品
 *    「背包问题」：dp(i - 1, W - weight) // 放入第i种物品后，则不能再使用该种物品了
 */
