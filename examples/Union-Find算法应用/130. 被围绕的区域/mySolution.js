 ...
 * 2.任何不在边界上，且不与边界上的'O'相连的'O'最终都会被填充为'X'。
 * 
 * 解决这个问题的传统方法为：
 * 先将那些与可以边界通过'O'相连的'O'替换成特殊字符如'#'，然后再遍历整个棋盘，把剩下的'O'换成'X'，再把'#'恢复成'O'。时间复杂度为O(MN)。
 ...
 */
 var solve = function(board) {
  ...
  for(let i = 0; i <= m - 1; i++){
    for(let j = 0; j <= n - 1; j++){
      ...
      if(i === 0 || i === m - 1 || j === 0 || j === n - 1){
        board[i][j] === 'O' && uf.union(dummy, i * n + j + 1)
      }else{
        // 将此'O'与上下左右的'O'连通
        if(board[i][j] === 'O'){
          if(board[i][j + 1] === 'O') uf.union(i * n + j + 1, i * n + j + 2)
          if(board[i][j - 1] === 'O') uf.union(i * n + j + 1, i * n + j)
          if(board[i + 1][j] === 'O') uf.union(i * n + j + 1, (i + 1) * n + j + 1)
          if(board[i - 1][j] === 'O') uf.union(i * n + j + 1, (i - 1) * n + j + 1)
        }
      }
    }
  ...
  for(let i = 0; i <= m - 1; i++){
    for(let j = 0; j <= n - 1; j++){ 
      if(!uf.connected(i * n + j + 1, dummy) && board[i][j] === 'O') {
        board[i][j] = 'X'
      }
    }
  }
